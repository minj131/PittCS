...
class Game
  ...
  # Check an upper-right to lower-left diagonal starting at location
  # start_x, start_y
  def check_upper_right_diagonal(end_x, end_y)
    x = end_x
    y = end_y
    counter = 0
    current_player = nil
  ...
  # Check an upper-left to lower-right diagonal starting at location
  # start_x, start_y
  def check_lower_right_diagonal(start_x, end_y)
    x = start_x
    y = end_y
    counter = 0
    current_player = nil
  ...
  def diagonal_winner?
    ...
    # Look diagonally (upper-right to lower-left) along the top row
    # Return true if connect four found
    @size.times do|end_y|
      return true if check_upper_right_diagonal(@size-1, end_y)
    end

    # Look diagonally (lower-left to upper-left) along the left col
    # Return true if connect four found
    @size.times do |start_x|
      return true if check_lower_right_diagonal(start_x, @size-1)
    end
  ...
  def pieces_fall
    ...
    @size.times do |column|
      if @board[bottom_row][column] != BLANK_SPACE ||
        ...
        #TODO: Markers fall to the "floor" but in case of an empty space in between, 
        #the markers will stop.
